...
class VelocityLocalPlannerDataset(BaseImageDataset):
    def __init__(self,
            zarr_path, 
            horizon=1,
            pad_before=0,
        ...
        self.replay_buffer = ReplayBuffer.copy_from_path(
            zarr_path, keys=['img', 'state', 'action', 'goal'])
    ...
    def _sample_to_data(self, sample):
        agent_vel = sample['state'].astype(np.float32) # T, 3
        target_point = sample['goal'].astype(np.float32) # T, 3
...
def test():
    ...
    dataset = VelocityLocalPlannerDataset(zarr_path, horizon=16)
